...
from hr.constants import MAX_MONTH_DAYS, MAX_WORKING_DAYS, MAX_HOLIDAY_DAYS, MAX_VACATION_DAYS
...
class SalarySerializer(serializers.Serializer):
    ...
    def validate_sick_days(self, value):
        if value > 3:
            raise serializers.ValidationError('The number of sick days cant be more than 3.')
        return value

    def validate_holiday_days(value):
        if value > MAX_HOLIDAY_DAYS:
            raise serializers.ValidationError(f'The number of holiday days cant be more than {MAX_HOLIDAY_DAYS}.')
        return value

    def validate_vacation_days(self, value):
        if value > MAX_VACATION_DAYS:
            raise serializers.ValidationError(f'The number of vacation days cant be more than {MAX_VACATION_DAYS}.')
        return value