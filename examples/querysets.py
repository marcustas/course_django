...
def querysets_examples(request):
    ...
    # OR (|)
    and_positions = Position.objects.filter(
        Q(is_active=True) | Q(is_manager=False)
    )
    and_positions = Position.objects.filter(is_active=True, is_manager=False)
    or_positions = Position.objects.filter(
        is_active=True
    ) | Position.objects.filter(is_manager=False)
    ...
    positions_containing_manager = Position.objects.filter(
        title__icontains='teac'
    )
