...
class EmployeeUpdateView(UserIsAdminMixin, UpdateView):
    ...
    def form_valid(self, form):
        response = super().form_valid(form)
        messages.success(self.request, 'Інформацію про працівника успішно змінено.')
        return response

    def form_invalid(self, form):
        messages.error(self.request, 'Виникла помилка при зміні інформації про працівника.')
        return super().form_invalid(form)
