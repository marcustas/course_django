...
from hr.models import Company, Department, Position, Employee
...
def homework_querysets(request):
    ...
    'first_queryset': json.loads(serialize('json', Department.objects.filter(position__title__icontains='manager').order_by('name'))),
    ...
    'third_queryset': json.loads(serialize('json', Position.objects.filter(Q(is_active=True) | Q(department__name='HR')))),
