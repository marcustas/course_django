...
class SalaryForm(forms.Form):
    ...
    def clean_employee(self):
        ...
        if not employee or employee is None:
            raise forms.ValidationError('Fill in the field "Employee"')
        return employee
    ...
    def clean_days(self):
        ...
        if sick_days is not None and sick_days > 5:
            raise forms.ValidationError('There should be less than 6 sick days')

        vacation_days = list(cleaned_data.values()).count('HOLIDAY')
        if vacation_days is not None and vacation_days > 3:
            raise forms.ValidationError('There must be less than 4 days off')
